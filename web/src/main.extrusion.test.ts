...
describe('shape movement', () => {
  ...
  it('M moves a hovered rectangle with a left-drag and Enter commits one undoable edit', () => {
    ...
    runFrame();
    expect(h.hudKeys.at(-1)).toEqual([
      { key: 'Space', label: 'draw' },
      { key: 'Shift', label: 'orbit' },
      { key: 'L', label: 'size' },
      { key: 'H', label: 'help' },
    ]);
  ...
  const chromeTarget = {
    closest: (selector: string) => selector === '[data-cad-ui]' ? {} : null,
  };

  it.each([
    ['an input', chromeTarget],
    ['a textarea', chromeTarget],
    ['a contenteditable element', chromeTarget],
...
describe('triangle and prism workflows', () => {
  ...
  it('keeps compact HUD chips for a triangle and still allows Q, M, and prism depth', () => {
...
describe('FreeCAD export snapshots', () => {
  ...
  it('keeps empty-sketch, typing, and overlay protections', () => {
    ...
    api.commands.addLine(v3(0, 0, 0), v3(10, 20, 30));
    const chrome = { closest: (selector: string) => selector === '[data-cad-ui]' ? {} : null };
    pressE({ target: chrome });
    pressE({ target: chrome });
    h.measure.isOpen = true;
    pressE();
    h.measure.isOpen = false;
