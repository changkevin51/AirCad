...
class App {
  ...
  private clearDrawHold(): void {
    for (const [source, action] of this.holdSources) {
      if (action === 'draw') this.holdSources.delete(source);
    }
  ...
  private beginExtrusion(): void {
    ...
    if (!target || target.type === 'line') {
      this.toasts.show('Select a closed rectangle or triangle: point inside it and pinch, click, or press S.', 'error', 5000);
      return;
    }
  ...
  private async exportToFreeCad(): Promise<void> {
    ...
    try {
      ...
      if (preview?.type === 'extrusion' && Math.abs(preview.depth) < 1e-6) {
        preview = { id: preview.id, type: 'rect', corners: preview.corners };
  ...
  private endStroke(): void {
    ...
    if (resolution.status !== 'ready') {
      if (resolution.status === 'unrecognized') {
        ...
        this.toasts.show('Not recognized: draw a straight line, a closed rectangle, or a triangle', 'error');
      } else {
  ...
  private applyGhost(resolution: GuidedStrokeResolution): void {
    ...
    if (input.type !== 'line' && input.type !== 'rect' && input.type !== 'triangle') {
      this.sketchRenderer.setGhost(null, false, null);
      return;
    }
  ...
  private keyHints(mode: Mode): KeyHint[] {
    ...
    switch (mode) {
      ...
      default: {
        ...
        if (target && target.type !== 'line') hints.push({ key: key('extrude'), label: target.type === 'triangle' ? 'extrude triangle' : 'push/pull' });
        if (target) hints.push({ key: key('move'), label: 'move' });
        if (target) hints.push({ key: key('scale'), label: 'scale' });
        if (target) {
          if (target.type !== 'triangle') hints.push({ key: key('measure'), label: target.type === 'extrusion' || target.type === 'prism' ? 'depth' : 'size' });
