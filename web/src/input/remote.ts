...
/**
 * Button to `KeyboardEvent.code`.  F17-F20 are unbound in the keymap and are not
 * printable, so a button can never type into the measure dialog or the
 * inspector.  Deliberately not F13-F16: macOS assigns F13 to Print Screen and
 * F14/F15 to screen brightness, so those arrive as system actions rather than as
 * key events.  F17-F20 carry no default macOS or Windows binding, and unlike
 * F21+ they still have macOS virtual key codes, so the browser reports them as
 * proper `code` values.  Keep in step with `buttonKeys` in the firmware.
 */
